...
export default function VotingContainer({ handleAlert }: { handleAlert: Function }) {
  ...
  const handleVote = () => {
    if (rating === 0) {
      handleAlert("Please select a rating.")
    } else {
      setHasVoted(true);
    }
  }
  ...
  if (hasVoted === false) {
    return (
      <div className="bg-gradient-to-b from-fe-gradient-light to-fe-gradient-dark w-[340px] md:w-[400px] h-[400px] rounded-[30px] shadow-md p-10 flex flex-col justify-between">
        ...
        <VotingRatingButtons rating={rating} onClick={setRating} />
        <VotingSubmitButton onClick={handleVote} />
      </div>
    )
  } else {
    return (
